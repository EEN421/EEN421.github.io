...
Five briefs, **21 detection candidates** — 4 production, 6 hunting-only, 11 needing environment mapping — and Thursday and Friday were almost entirely one story: **ChainDrop**, a self-propagating npm worm in 400+ compromised packages, stealing registry and CI credentials from build hosts and routing its C2 through **Ethereum smart contracts**. Nine candidates across the two days, plus the `keyv`/`cacheable` worm from Wednesday's SANS diary, plus a Rails file read, Midnight Blizzard, AMOS, an N-central auth bypass, and a Kerberoast.
...
Friday's Detection 1 wants to catch ChainDrop stealing GitHub Actions runner secrets. Here is its core:
...
`curl` is npm's **grandchild**. Its parent is `sh`. Friday's query looks one level down from npm and finds `sh`; it looks for `curl` and demands that `curl`'s parent be npm. Neither test can be satisfied by the actual process tree. Thursday's Detection 4 and Friday's Detection 3 are honest one-level queries and at least return the `node → sh` edge — but the edge they return is the *scaffolding*, not the payload. The interesting process is one generation further down than any query in the week can reach.
...
Thursday's Detection 4 tries to suppress benign toolchain noise:
...
Friday's Detection 2 goes after it like this:
...
Wednesday's Detection 3 defines suspicious C2 as an outbound connection **without DNS resolution** — `isempty(RemoteUrl)` or a `RemoteUrl` that's a bare IP literal, from a scripting interpreter, three or more times.
...
- **T1071.001 / T1090.001 on a credential-access detection.** Friday's Detection 1 hunts secret extraction on a runner and is mapped entirely to Command and Control techniques. The behaviour is T1552 (what was taken) and T1195.002 (how they got there); an RPC provider is not an internal proxy under any reading of T1090.001.
- **T1195 filed under Impact.** Thursday's Detection 2 lists "Impact: T1195 Supply Chain Compromise". T1195 is an **Initial Access** technique with no Impact variant. Tactic errors are worse than technique errors in a coverage map, because they misreport which *phase* you can see.
- **T1071.001 on both Wednesday's direct-to-IP C2 and Friday's blockchain C2.** Two detections whose definitions of C2 are mutually exclusive — one requires the absence of name resolution, the other is entirely name-resolved — reporting into the same cell. Both mappings are defensible; the aggregate is not.
